...
REFERENCE_FILE = Path(__file__).parent / "data" / "test_reference.csv"
...
if "df_train" not in st.session_state:
    st.session_state.df_train = (
        pd.read_csv(REFERENCE_FILE) if REFERENCE_FILE.exists() else None
    )
...
if df_train is not None and "proba" in df_train.columns and "predicted_label" in df_train.columns:
    st.markdown('<p class="section-title">📈 Position du client dans la population de référence</p>', unsafe_allow_html=True)
    ...
    fig_hist.update_layout(
        **PLOTLY_LAYOUT, margin=DEFAULT_MARGIN,
        ...
        title=dict(text=f"Distribution des probabilités — {len(df_train):,} clients (référence)",
    ...
    if shap_values and len(train_shap_cols) >= 2:
        ...
        if feat1_shap in df_train.columns and feat2_shap in df_train.columns:
            st.markdown('<p class="section-title">🔬 Scatter — Top 2 features SHAP (vs référence)</p>', unsafe_allow_html=True)
            ...
            for lbl, sub, color in [("Acceptés", df_acc, COLOR_ACCEPT), ("Rejetés", df_rej, COLOR_REJECT)]:
                fig_scatter.add_trace(go.Scattergl(
                    ...
                    mode="markers", name=f"{lbl} (référence)",
