...
**Option B — Modèles lourds : stockage externe**

Uploadez vos `.pkl` sur un bucket S3, Google Cloud Storage ou un lien de téléchargement direct.  
Ajoutez un script `download_models.py` :
...
urllib.request.urlretrieve(
    "https://votre-url.com/LightGBM_best_model.pkl",
    "models/LightGBM_best_model.pkl"
)
urllib.request.urlretrieve(
    "https://votre-url.com/XGBoost_best_model.pkl",
    "models/XGBoost_best_model.pkl"
)
```

Et renseignez comme **Start Command** dans Render :

```
python download_models.py && uvicorn main:app --host 0.0.0.0 --port $PORT
```
