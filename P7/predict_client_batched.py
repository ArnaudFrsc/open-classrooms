...
OUTPUT_DIR = r'C:\Users\jfurs\Pythonn\OpenClassrooms\DS\P7\output_data'

with open(f'{OUTPUT_DIR}/cols_kept.json') as f:
...
INPUT_FILE   = f'{OUTPUT_DIR}\\api_test_sample.csv'   # Fichier CSV ou Excel
OUTPUT_FILE  = f'{OUTPUT_DIR}\\predictions_explained_train.csv' # Fichier de sortie
...
def _load_and_align(filepath: Path) -> Path:
    ...
    print(f"🔧 Pré-traitement : {len(df)} lignes alignées sur {len(cols)} colonnes")
    return df
...
def predict_explain(output_file: str | None = None) -> None:
    ...
    df_full = _load_and_align(filepath)
